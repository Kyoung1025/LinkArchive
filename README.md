...
배운 개념은 진도 나간 그 주 안에 바로 코드로 적용하는 걸 원칙으로 합니다 (밀리면 개념이 휘발됨).
...
### 3단계 — 커리큘럼 밖, 자율 학습 (Docker/K8s 실습이 어느 정도 끝난 뒤)

CI/CD는 부트캠프 커리큘럼에 없을 가능성이 커서 별도로 시간을 떼어놓아야 합니다.

- [ ] GitHub Actions: PR에서 lint/타입체크/빌드
- [ ] merge 시 이미지 빌드 + 배포 자동화

### 4단계 — 마무리 (부트캠프 종료 전, 늦어도 종료 시점까지)
