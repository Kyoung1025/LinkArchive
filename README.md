...
**목적**: 이 프로젝트는 클라우드 인프라 엔지니어 취업 포트폴리오입니다. 핵심은 기능의 화려함이 아니라, **비동기 워커 큐 아키텍처**(API 서버·Redis 큐·워커를 독립적으로 배포 가능한 프로세스로 분리)를 프로덕션급으로 설계해서, kt cloud K-Digital Training 커리큘럼에 맞춰 Docker → Terraform → Kubernetes 순으로 클라우드 인프라 학습을 단계적으로 얹을 수 있게 만드는 것입니다.
